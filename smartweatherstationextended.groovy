...
metadata {
	...
	definition (name: "SmartWeather Station Tile Extended", namespace: "sidjohn1", author: "SmartThings") {
		...
		attribute "forecastIcon1", "string"
        attribute "forecastHigh1", "string"
        attribute "forecastLow1", "string"
		attribute "forecastIcon2", "string"
        attribute "forecastHigh2", "string"
        attribute "forecastLow2", "string"
		attribute "forecastIcon3", "string"
        attribute "forecastHigh3", "string"
        attribute "forecastLow3", "string"
	...
	tiles {
		...
		valueTile("moon", "moonPhase", width: 3, height: 1, decoration: "flat") {
			state "default", label:'${currentValue}'
		}

		main(["temperature", "weatherIcon","feelsLike"])
		details(["temperature", "humidity", "weatherIcon","feelsLike","wind","weather", "city","percentPrecip", "refresh","alert","rise","set","light","moon"])}
}
...
def poll() {
	...
	if (obs) {
        ...
        if (a) {
        send(name: "moonPhase", value: a.phaseofMoon as String)
        }
		...
		if (f1) {
            ...
            def icon1 = f1[1].icon_url.split("/")[-1].split("\\.")[0]
            def icon2 = f1[2].icon_url.split("/")[-1].split("\\.")[0]
            def icon3 = f1[3].icon_url.split("/")[-1].split("\\.")[0]
            ...
            send(name: "forecastIcon1", value: icon1, displayed: false)
            send(name: "forecastIcon2", value: icon2, displayed: false)
            send(name: "forecastIcon3", value: icon3, displayed: false)
            
            if(getTemperatureScale() == "C") {
            send(name: "forecastHigh", value: Math.round(f1[0].high.celsius as Double), unit: "C")
			send(name: "forecastLow", value: Math.round(f1[0].low.celsius as Double), unit: "C")
			send(name: "forecastHigh1", value: Math.round(f1[1].high.celsius as Double), unit: "C")
			send(name: "forecastLow1", value: Math.round(f1[1].low.celsius as Double), unit: "C")
            send(name: "forecastHigh2", value: Math.round(f1[2].high.celsius as Double), unit: "C")
			send(name: "forecastLow2", value: Math.round(f1[2].low.celsius as Double), unit: "C")
            send(name: "forecastHigh3", value: Math.round(f1[3].high.celsius as Double), unit: "C")
			send(name: "forecastLow3", value: Math.round(f1[3].low.celsius as Double), unit: "C")
			} 
        	else {
            send(name: "forecastHigh", value: Math.round(f1[0].high.fahrenheit as Double), unit: "F")
			send(name: "forecastLow", value: Math.round(f1[0].low.fahrenheit as Double), unit: "F")
			send(name: "forecastHigh1", value: Math.round(f1[1].high.fahrenheit as Double), unit: "F")
			send(name: "forecastLow1", value: Math.round(f1[1].low.fahrenheit as Double), unit: "F")
            send(name: "forecastHigh2", value: Math.round(f1[2].high.fahrenheit as Double), unit: "F")
			send(name: "forecastLow2", value: Math.round(f1[2].low.fahrenheit as Double), unit: "F")
            send(name: "forecastHigh3", value: Math.round(f1[3].high.fahrenheit as Double), unit: "F")
			send(name: "forecastLow3", value: Math.round(f1[3].low.fahrenheit as Double), unit: "F")
			}
